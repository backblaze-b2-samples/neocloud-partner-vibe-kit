...
- [ ] Operator master key is **never** used as an S3 credential by any tenant or tool. Backblaze does not restrict the master key from the S3 API at the protocol level — the restriction is policy you enforce. Verified by `grep` over CI configs, deployment manifests, and S3 client config files.
